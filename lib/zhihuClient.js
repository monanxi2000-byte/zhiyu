...
function resolveCliPath() {
  ...
  // 2. 项目内置 Linux 版 CLI（部署环境，bin/zhihu-cli）
  if (process.platform !== 'win32') {
    const bundled = path.join(__dirname, '..', 'bin', 'zhihu-cli');
    if (fs.existsSync(bundled)) {
      try { fs.chmodSync(bundled, 0o755); } catch (e) {}
      return bundled;
    }
    // 3. postinstall / 启动脚本安装的 CLI
    const localInstall = path.join(__dirname, '..', '.zhihu-cli', 'zhihu-cli');
    if (fs.existsSync(localInstall)) {
      try { fs.chmodSync(localInstall, 0o755); } catch (e) {}
      return localInstall;
    }
  }
...
function normalizeList(out) {
  ...
  if (out && out.Data && out.Data.Items && Array.isArray(out.Data.Items)) return out.Data.Items;
  if (out && out.data && out.data.items && Array.isArray(out.data.items)) return out.data.items;
  return [];
}
